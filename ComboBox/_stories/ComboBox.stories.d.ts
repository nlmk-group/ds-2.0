import { default as React } from '../../../../node_modules/react';

declare const _default: {
    title: string;
    component: (props: import('../types').IComboBoxProps) => React.JSX.Element;
    argTypes: {
        label: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: string;
        };
        isDisabled: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: boolean;
        };
        isResize: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: boolean;
        };
        dropdownWidth: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: string;
        };
        dropdownHeight: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: string;
        };
        dropdownMinHeight: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: number;
        };
        dropdownMinWidth: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: number;
        };
        inputMinWidth: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: number;
        };
        initialValue: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: never[];
        };
        children: {
            description: string;
            control: {
                type: string;
            };
        };
        displayEnumerated: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: boolean;
        };
        displayChildCount: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: boolean;
        };
        tooltipDescription: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: string;
        };
        countOnlyLevel: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: number;
        };
        color: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: string;
        };
        className: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: string;
        };
        style: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: {};
        };
        dropdownClassName: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: string;
        };
        dropdownStyle: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: {};
        };
        inputClassName: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: string;
        };
        inputStyle: {
            description: string;
            control: {
                type: string;
            };
            defaultValue: {};
        };
    };
    decorators: ((Story: any) => React.JSX.Element)[];
};
export default _default;
export declare const BasicSingleSelect: {
    (): React.JSX.Element;
    storyName: string;
};
export declare const BasicSingleSelectWithInitialValue: {
    (): React.JSX.Element;
    storyName: string;
};
export declare const MultipleSelectWithInitialValue: {
    (): React.JSX.Element;
    storyName: string;
};
export declare const MultipleSelectWithSearchAndCheckAll: {
    (): React.JSX.Element;
    storyName: string;
};
export declare const DraggableListExample: {
    (): React.JSX.Element;
    storyName: string;
};
export declare const GroupedOptionsExample: {
    (): React.JSX.Element;
    storyName: string;
};
export declare const TreeViewExample: {
    (): React.JSX.Element;
    storyName: string;
};
export declare const TreeViewWithMaxLevelExample: {
    (): React.JSX.Element;
    storyName: string;
};
export declare const ResizableDropdownExample: {
    (): React.JSX.Element;
    storyName: string;
};
export declare const TooltipDescriptionExample: {
    (): React.JSX.Element;
    storyName: string;
};
export declare const TooltipDescriptionStringExample: {
    (): React.JSX.Element;
    storyName: string;
};
export declare const TreeViewWithSimpleCheckboxesExample: {
    (): React.JSX.Element;
    storyName: string;
};
//# sourceMappingURL=ComboBox.stories.d.ts.map