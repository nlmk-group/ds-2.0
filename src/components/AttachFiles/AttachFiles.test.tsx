...
import { File, AttachFiles } from '@components/index';
import { render, screen, fireEvent } from '@testing-library/react';
import { FILES_DESCRIPTION, FILE_NAME, LONG_LABEL } from './_stories/text';
  ...
  describe('While rendering left icon', () => {
    ...
    describe('While rendering checkbox', () => {
      ...
      const FileWithCheckBox = () => (
        <File
          label={FILE_NAME}
          checkedValue={checkedValueTrue}
          checked
          onCheckedChange={mockCallBack}
        />
      );
    ...
    describe('While rendering with control', () => {
      describe('While rendering with removed control', () => {
        const mockCallBack = jest.fn();
        const FileWithRemovedControl = () => (
          <File
            label={FILE_NAME}
            removed
            removedOnClick={mockCallBack}
          />
        );
      ...
      describe('While rendering with commented control', () => {
        const mockCallBack = jest.fn();
        const FileWithCommentedControl = () => (
          <File
            label={FILE_NAME}
            commented
            commentedOnClick={mockCallBack}
          />
        );
        ...
        test('It should call function on click action', () => {
          ...
          const commentedBtn = screen
            .getByTestId('COMMENTED_CONTROL')
            .querySelector('button') as HTMLInputElement;
      ...
      describe('While rendering with commented control', () => {
        const mockCallBack = jest.fn();
        const FileWithSavedControl = () => (
          <File
            label={FILE_NAME}
            saved
            savedOnClick={mockCallBack}
          />
        );
      ...
      describe('While rendering with commented control', () => {
        const mockCallBack = jest.fn();
        const FileWithAddFileControl = () => (
          <File
            label={FILE_NAME}
            addFile
            addFileOnClick={mockCallBack}
          />
        );
        ...
        test('It should call function on click action', () => {
          ...
          const addFileBtn = screen
            .getByTestId('ADD_FILE_CONTROL')
            .querySelector('button') as HTMLInputElement;
    ...
    test('It should render an AttachFile component with correct amount of Files components', () => {
      ...
      render(
        <AttachFiles>
          {FileList.map((FileItem, index) => (<Fragment key={index}>{FileItem}</Fragment>))}
        </AttachFiles>
      );
