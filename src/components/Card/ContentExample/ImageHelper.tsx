...
const ImageHelper: FC<IImageHelper> = ({
  orientation,
  imageURL,
  badges
}) => {
  return (
    <div className={clsx(styles['image-wrapper'], styles[`image-${orientation}`])}>
      {imageURL.length ? (
        ...
        <div
          data-testid='IMAGE_BLANK'
          className={styles['icon-wrapper']}
        >
          <Icon
            name='IconFactory32'
            htmlColor='var(--dark-primary-navyblue-200)'
          />
        </div>
      )}
      {badges.length !== 0 && (
        <div
          data-testid='BADGES_WRAPPER'
          className={styles['badges-wrapper']}
        >
