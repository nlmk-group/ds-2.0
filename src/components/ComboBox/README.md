...
> **Внешние компоненты:** Используются компоненты из библиотеки `@components/index` (например, Checkbox, Box, Icon, Spinner, Typography и др.).

---

**Примечание:** Для сложных сценариев (например, кастомный рендеринг опций, интеграция с формами, асинхронная подгрузка) используйте дочерние компоненты и контекстные хуки.
