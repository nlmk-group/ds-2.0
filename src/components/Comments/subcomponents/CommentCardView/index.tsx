...
export const CommentCardView = ({
  ...
  const hasActions = !!comment.builtInActions?.onEdit ||
    !!comment.builtInActions?.onDelete ||
    !!comment.customActions?.length;
        ...
        <Grid.Column st={{ gap: 4 }}>
          {comment.commentLink && (
            <Grid.Column style={{ marginBottom: 4 }}>
              <Link href={comment.commentLink.url} target="_blank" size="body1">{comment.commentLink.label}</Link>
            </Grid.Column>
          )}
          <Grid.Column
            st={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}
          >
            {comment.badge?.label && (
              <Badge color={comment.badge.color} variant={comment.badge.variant}>{comment.badge.label}</Badge>
            )}
