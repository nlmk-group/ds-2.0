...
import { IUseIsStartYearParams } from '@components/DatePicker/subcomponents/YearsCalendar/types';

export const useIsStartYear = ({ withPeriod, dateFrom, dateCurrentHover }: IUseIsStartYearParams) =>
  useCallback(
    (dayAsDate: Date) => {
      if (!withPeriod) {
        return false;
