...
export const useIsStartYear = ({
  withPeriod,
  dateFrom,
  dateCurrentHover
}: useIsStartYearParams) =>
  useCallback(
    (dayAsDate: Date) => {
      if (!withPeriod) {
        return false;
      ...
      const numberedDateFrom =
        dateFrom && numberedDateByLevel(dateFrom, 'year');
      const numberedDayAsDate =
        dayAsDate && numberedDateByLevel(dayAsDate, 'year');
      const numberedDateCurrentHover =
        dateCurrentHover && numberedDateByLevel(dateCurrentHover, 'year');
