...
Компонент `DateTime` предназначен для отображения актуальных даты и времени с поддержкой локализации. Он автоматически обновляет время каждую минуту и форматирует дату в виде `ДД.ММ.ГГГГ, День недели` и время в формате `ЧЧ:ММ`. Компонент поддерживает русскую и английскую локализацию.
...
| --- | --- | --- | --- |
| locale | 'ru' \| 'en' | 'ru' | Задает язык для названия дня недели (например, Среда или Wednesday). Важно: Числовой формат даты (ДД.ММ.ГГГГ) фиксирован и не меняется. |
...
- Формат даты всегда `ДД.ММ.ГГГГ` (российский стандарт), независимо от выбранного языка.
- День недели выводится с заглавной буквы и зависит от выбранного языка.
