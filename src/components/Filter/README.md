...
| defaultFilterType | `string` | Начальный тип фильтра (если не задан, берётся первый из filterTypeOptions) | первый из options |
| withPortal | `boolean` | Нужно ли рендерить меню через портал? | `false` |
| portalContainerId | `string` | ID DOM-элемента для портала | `"root"` |
...
Если нужна дополнительная информация или примеры, обратитесь к исходному коду или напишите в поддержку библиотеки.
...
| reset    | `boolean`                                           | Показывать иконку сброса        | `false`      |
