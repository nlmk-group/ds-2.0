...
describe('src/components/Button', () => {
  ...
  test('Container size', () => {
    const sizes: number[] = [
      iconsSizesMapping.Small,
      iconsSizesMapping.Medium,
      iconsSizesMapping.Large
    ];
    sizes.map((size: number) => {
      const { container } = render(
        <Icon
          name={'Icon3Bunker16'}
          containerSize={size as TIconProps['containerSize']}
        />
      );
      const icon = container.getElementsByTagName('div')[0];
  ...
  test('Icon color', () => {
    ...
    colors.map((color: string) => {
      const { container } = render(
        <Icon name={'Icon3Bunker16'} color={color as TIconProps['color']} />
      );
  ...
  test('Html color', () => {
    ...
    const { container } = render(
      <Icon name={'Icon3Bunker16'} htmlColor={htmlColor} />
    );
