...
- В свёрнутом виде текст обрезается до 3 строк через `-webkit-line-clamp` с многоточием.
...
- При `unread=true` слева от timestamp рендерится красная точка-индикатор (`--spectrum-red-60`).
