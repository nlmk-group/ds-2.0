...
const scrollableMenuItems = Array.from({ length: 14 }, (_, index) => ({
  path: `Раздел ${index + 1}`,
  label: `Раздел меню ${index + 1}`,
  icon: SCROLLABLE_MENU_ICONS[index % SCROLLABLE_MENU_ICONS.length]
}));

export const SidebarVerticalScrollable = (argTypes: ISidebarProps): JSX.Element => {
  const [currentPath, setCurrentPath] = useState('Раздел 1');
...
SidebarVerticalScrollable.storyName = 'Вертикальный сайдбар с прокруткой меню';
SidebarVerticalScrollable.args = {
  isLoggedIn: true,
  userName: 'Иван',
  userSurname: 'Иванов',
  systemName: 'НЛМК'
