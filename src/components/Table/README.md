...
<Table horizontalBorders verticalBorders>
  <Thead>
    <Row>
      <Top title="Name" sort sortDirection="asc" filtered>
        <Filter
          defaultValue=""
...
Для кастомизации можно использовать свойство `className` и переопределять стили через CSS-модули или data-атрибуты, например, для кастомизации Cell можно использовать data-атрибут `[data-ui-cell-content]`.
