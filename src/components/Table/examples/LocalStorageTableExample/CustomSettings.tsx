...
export const CustomSettings = <T extends object>({
  ...
  useEffect(() => {
    if (!isInitialized.current) {
      defaultSettingsRef.current = {
        visibleColumns: defaultVisibleColumns || visibleColumns,
        columnOrder: defaultColumnOrder || columnOrder,
        pinnedColumns: {}
      };
    }
  }, [defaultVisibleColumns, defaultColumnOrder, visibleColumns, columnOrder]);
   ...
   */
  const processColumnsRecursive = useCallback((cols: any[], ids: string[], visibility: Record<string, boolean>) => {
    cols.forEach(col => {
      if (col.id) {
        ids.push(col.id);
  ...
  const countChanges = useCallback(() => {
    ...
    const getVisibilityChangeCount = (id: string) => {
      ...
      const initialValue = defaultSettingsRef.current.visibleColumns[id] ?? true;
      const currentValue = visibleColumns[id] ?? true;

      if (initialValue !== currentValue) {
        const parentId = childParentMap[id];
        if (parentId) {
          const parentInitial = defaultSettingsRef.current.visibleColumns[parentId] ?? true;
    ...
    Object.keys({ ...defaultSettingsRef.current.visibleColumns, ...visibleColumns }).forEach(id => {
      visibilityChanges += getVisibilityChangeCount(id);
    });

    if (columnOrder.join(',') !== defaultSettingsRef.current.columnOrder.join(',')) {
      orderChanged = true;
    }

    const allPinKeys = new Set([
      ...Object.keys(defaultSettingsRef.current.pinnedColumns),
      ...Object.keys(pinnedColumns)
    ]);

    allPinKeys.forEach(key => {
      const initialValue = defaultSettingsRef.current.pinnedColumns[key] || false;
  ...
  const handleResetToDefault = () => {
    ...
    const defaultVisibleCols = visibility;
    const defaultColOrder = ids;
