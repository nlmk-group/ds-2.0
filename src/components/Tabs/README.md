...
Компонент сам отслеживает переполнение через `ResizeObserver`: если суммарная ширина табов превышает ширину контейнера, автоматически появляются стрелки навигации и активируется горизонтальный скролл. Разработчику не нужно явно передавать проп `scrollable` в обычных случаях — всё работает из коробки.

Проп `scrollable` оставлен для принудительного включения поведения в специфических случаях (например, когда табы размещены в узком контейнере с собственным `overflow`, где автодетект может не сработать корректно).
