...
### Логика ограничений (улучшенная версия)

Компонент использует два параметра для точного контроля перетаскивания:

**1. `dropPosition` (из rc-tree)** - позиция сброса:

- `-1`: сброс **выше** целевого узла (на том же уровне)
- `0`: сброс **внутрь** узла (создание дочернего элемента)
- `1`: сброс **ниже** целевого узла (на том же уровне)

**2. Уровень глубины** - вычисляется через `getNodeLevel()`

**Правила ограничений:**

❌ **Запрещено:**

- Создание дочерних элементов (`dropPosition = 0`) - всегда
- Перемещение между разными уровнями глубины

✅ **Разрешено:**

- Перестановка рядом (`dropPosition = -1, 1`) между узлами одного уровня

**Преимущества:**

- Блокирует создание неправильных иерархических связей
- Использует нативные возможности rc-tree
- Соответствует ожиданиям UX
